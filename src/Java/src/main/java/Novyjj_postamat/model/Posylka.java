 ...
 */
@Entity(name = "IISNovyjj_postamatПосылка")
...
public class Posylka {
    ...
    @EdmIgnore
    @Converter(converterClass = UUIDConverter.class, name = "Klyuch")
    @Convert("Klyuch")
    @Column(name = "Ключ", length = 16, unique = true, nullable = false)
    private UUID _klyuchid;

    @ManyToOne(optional = false, fetch = FetchType.LAZY)
    @JoinColumn(name = "Klyuch", insertable = false, updatable = false)
    private Klyuch klyuch;
